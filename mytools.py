...
def contour_coors_to_curvature(contour):
    length = len(contour)
    result = [0] * len(contour)
    for i in xrange(len(contour)):
        x, y, z = contour[i-1], contour[i], contour[(i+1) % length]
        a = np.linalg.norm(x - y)
        b = np.linalg.norm(z - y)
        c = np.linalg.norm(x - z)

        result[i] = curvature(a, b, c)
...
def contour_change_length(contour, length):
    if len(contour) > length:
        contour = contour_reduce(contour, length)
    else:
        contour = contour_extend(contour, length)
 
    return contour
 
def contours_compare(a, b):
    length = min([100, int((len(a) + len(b)) / 2)])
 
    a = contour_change_length(a, length)
    b = contour_change_length(b, length)
...
if __name__ == '__main__':
    ...
    cv2.drawContours(objects, palms, -1, (0, 255, 0), 2)
    cv2.imwrite('palms.bmp', objects)